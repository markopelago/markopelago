<?php  
	...
	if(!$user_id){
		$_SESSION["errormessage"] = v("phone_confirmation_token_expired");
	} else {
		$db->addtable("a_users");	$db->where("id",$user_id);
		$db->addfield("token");		$db->addvalue("");
		$db->addfield("phone_confirmed_at");$db->addvalue($__now);
		$updating = $db->update();
		if($updating["affected_rows"] > 0){
			...
			$_SESSION["message"] = v("phone_confirmed");
		} else {
			$_SESSION["errormessage"] = v("phone_confirmation_failed");
		}
	}
