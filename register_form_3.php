<?php 
	if($_GET["as"] == "seller"){
		...
		if(isset($_POST["next"])){
			$error 		= "";
			$_type 		= $_FILES["header"]["type"];
			$_size 		= $_FILES["header"]["size"];
			$_error 	= $_FILES["header"]["error"];
			$_filename 	= basename($_FILES["header"]["name"]);
			...
			if($_size > (4*1048576)) { $error = str_replace("{max_file}",4,v("image_to_big")); }
			if($error == ""){
				$filetemp = "header".rand(0,9).rand(0,9).rand(0,9).rand(0,9).rand(0,9).rand(0,9).rand(0,9).$__user_id.".".$_file_ext;
				if (move_uploaded_file($_FILES["header"]["tmp_name"], "users_temp/".$filetemp)){
...
<!--form id="frmUploadPhoto" role="form" method="POST" autocomplete="off" enctype="multipart/form-data"-->
<form action="register.php?step=3" method="POST" enctype="multipart/form-data">
	<input type="hidden" name="mode_photo" value="header">
			...
			<div class="input-group">
				<span class="input-group-btn">
					<span class="btn btn-default btn-file">
						<?=v("browse");?>... <input type="file" name="header" id="imgInp" accept="image/*">
					</span>
				</span>
